...
function ensureProbeTopicDialog() {
  if (byId("probeTopicDialog")) return;
  const dialog=document.createElement("dialog");
  dialog.id="probeTopicDialog";
  dialog.className="probe-topic-dialog";
  dialog.innerHTML=`<form method="dialog" class="probe-topic-form"><p class="eyebrow">Probe abschließen</p><h2>Thema der Probe</h2><p>Bitte das Thema der ${escapeHtml(sessionType)} eintragen. Das Thema erscheint in CSV, PDF und Archiv.</p><label for="probeTopicInput">Thema *</label><textarea id="probeTopicInput" maxlength="300" rows="4" required placeholder="z. B. Löschangriff, Funkübung oder Fahrzeugkunde"></textarea><p class="probe-topic-error" id="probeTopicError" hidden>Bitte ein Thema eingeben.</p><div class="probe-topic-actions"><button type="button" class="outline-button" id="cancelProbeTopicButton">Abbrechen</button><button type="button" class="primary-button" id="confirmProbeTopicButton">Weiter zum Abschluss</button></div></form>`;
