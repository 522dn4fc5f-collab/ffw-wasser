...
function ensureSettingsNavigation(){
  if(!byId("settingsTab")){
    const button=document.createElement("button");button.id="settingsTab";button.className="compact-menu-button";button.type="button";button.textContent="Einstellungen";
    const menu=byId("compactMenu"),admin=byId("adminTab");menu.insertBefore(button,admin||menu.firstChild);
    button.addEventListener("click",()=>showView("settingsView"));
  }
  if(!byId("settingsView")){
    const section=document.createElement("section");section.id="settingsView";section.className="view settings-view";section.hidden=true;
    section.innerHTML=`<div class="screen-heading"><div><p class="eyebrow">Übersicht</p><h2>Einstellungen</h2><p>Wähle einen Bereich. Jeder Bereich öffnet eine eigene Unterseite.</p></div><button class="outline-button view-home-button" data-settings-home type="button">Zurück zu Home</button></div><div class="settings-group"><div class="settings-group-heading"><span>Verwaltung</span><h3>Personen und Vorgaben</h3></div><div class="settings-menu-grid settings-menu-grid-management"><article class="panel settings-menu-card"><span class="step yellow">PERSON</span><h3>Mitglieder</h3><p>Mitglieder, Funktionen und Fahrzeugberechtigungen verwalten.</p><button class="primary-button" data-settings-page="settingsMembersView" data-protected type="button">Mitglieder öffnen</button></article><article class="panel settings-menu-card"><span class="step yellow">ZIEL</span><h3>Funktionsziele</h3><p>Jährliche Mindestziele für die einzelnen Funktionen festlegen.</p><button class="primary-button" data-settings-page="settingsGoalsView" data-protected type="button">Ziele öffnen</button></article><article class="panel settings-menu-card"><span class="step yellow">PIN</span><h3>Sicherheit</h3><p>Die Admin-PIN für geschützte Bereiche ändern.</p><button class="primary-button" data-settings-page="settingsSecurityView" data-protected type="button">Sicherheit öffnen</button></article></div></div><div class="settings-group"><div class="settings-group-heading"><span>Daten</span><h3>Berichte und Sicherung</h3></div><div class="settings-menu-grid settings-menu-grid-data"><article class="panel settings-menu-card"><span class="step yellow">AUSWERTUNG</span><h3>Statistik</h3><p>Teilnahmen und Funktionen auswerten.</p><button class="primary-button" data-settings-page="settingsStatisticsView" type="button">Statistik öffnen</button></article><article class="panel settings-menu-card"><span class="step yellow">HISTORIE</span><h3>Historie</h3><p>Abgeschlossene Proben nach Jahr und Monat aufrufen.</p><button class="primary-button" data-settings-page="settingsHistoryView" type="button">Historie öffnen</button></article><article class="panel settings-menu-card"><span class="step yellow">DATEI</span><h3>Speicherorte und Backups</h3><p>PDF-Speicherort und vollständige Sicherungen verwalten.</p><button class="primary-button" data-settings-page="settingsFilesView" data-archive-protected type="button">Speicherorte öffnen</button></article></div></div>`;
    document.querySelector("main.app-shell").appendChild(section);
    section.querySelector("[data-settings-home]").addEventListener("click",()=>showView("attendanceView"));
    section.querySelectorAll("[data-settings-page]").forEach(button=>button.addEventListener("click",()=>{
      const target=button.dataset.settingsPage;
      pendingSettingsTarget=target;
      if(button.hasAttribute("data-protected")&&!adminUnlocked)return showView("adminLoginView");
      if(button.hasAttribute("data-archive-protected")&&!adminUnlocked)return showView("archiveLoginView");
      showView(target);
...
function updateHelpForCurrentFeatures(){
  ...
  if(completion) completion.innerHTML=`<h3>Probe in drei Schritten durchführen</h3><p><strong>Schritt 1:</strong> Probenart auswählen und „Weiter zu Schritt 2“ anklicken. <strong>Schritt 2:</strong> Über die Aktenreiter Anwesend, Entschuldigt und bei Sonderproben Betrifft nicht auswählen, Mitglieder markieren und speichern. „Anwesend“ ist voreingestellt. Mit „Weiter zu Schritt 3“ geht es weiter. <strong>Schritt 3:</strong> Bei einer Allgemeinen Probe erscheint die Taktik, bei Sonderprobe und Unterricht direkt der Abschluss. Beim Wechsel der Probenart werden Taktik und aktuelle Auswahl zurückgesetzt.</p><p>Der Kopfbereich zeigt nur während einer laufenden Probe die Probenart sowie die Anzahl anwesender und entschuldigter Personen. CSV, PDF und Archiv werden beim vollständigen Abschluss erzeugt beziehungsweise aktualisiert.</p>`;
