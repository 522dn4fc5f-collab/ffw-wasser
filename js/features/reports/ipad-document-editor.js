...
(function(){
  ...
  function drawEditor(){
    ...
    // Der Ausschnittrahmen wird zusätzlich direkt auf die Vorschau gezeichnet.
    // Dadurch bleibt er auch auf iPad/Safari sichtbar, falls die Dialog-Ebene
    // oder ein GPU-gerendertes Canvas die HTML-Rahmenebene überlagert.
    const {crop}=stageMetrics();ctx.save();ctx.setTransform(dpr,0,0,dpr,0,0);ctx.strokeStyle="#00e676";ctx.lineWidth=6;ctx.shadowColor="rgba(0,0,0,.85)";ctx.shadowBlur=5;ctx.strokeRect(crop.x+3,crop.y+3,Math.max(1,crop.width-6),Math.max(1,crop.height-6));ctx.restore();
